...
- リリース作成: `.agents/skills/release/SKILL.md`

この `.agents/skills/` が内容の正です。Claude Code は同内容の同期コピーを `.claude/skills/` から自動検出し、`/github-issue`・`/github-pr`・`/github-review`・`/github-release` として使えます。編集は `.agents/skills/` 側で行い、`.claude/skills/` へコピーして同期すること（詳細は `.claude/skills/README.md`）。GitHub PR のレビューは、指摘を該当行へインラインコメントで投稿するのを既定とします。
...
Releases へバイナリ配布を追加するときの手順は `.agents/skills/release/SKILL.md`（Claude Code では `/github-release`）にまとめています。要点だけ抜粋:

- タグは `vX.Y.Z`（SemVer）。`main` の最新コミットを対象にする。
- exe は **Python 3.12 専用 venv**＋PyInstaller onedir でビルドする（**3.14 は不可**）。
- リリース zip（`AIUsageTray-vX.Y.Z-windows-x64.zip`）に**個人の `config.json` を同梱しない**。`app.ico` は exe 同階層へ同梱する。
- リリースノートは絵文字なしで、既存リリース（v1.0.0 / v1.1.0 / v1.2.0）と同じ「主な変更 / インストール方法 / 動作要件 / 設定」構成にする。
