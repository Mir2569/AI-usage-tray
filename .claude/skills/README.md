...
`/github-issue`・`/github-pr`・`/github-review`・`/github-release` として使うにはここにファイルを置く必要があります。
...
- `github-release/SKILL.md`: GitHub Releases へのバイナリ配布。タグ命名・3.12 ビルド・config.json 非同梱・zip 添付・リリースノート書式。

## `.agents/skills/` との関係（同期メモ）

正の内容は `.agents/skills/{issue,pr,review,release}/SKILL.md` にあります（Codex など Claude Code 以外のエージェントが読む場所）。
