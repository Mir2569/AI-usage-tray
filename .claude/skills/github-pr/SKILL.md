...
- 対応する Issue を先に立て、PR 本文から `Closes #<番号>` で紐付ける。
- マージは基本的にユーザーが行う。こちらからはマージしない（明示依頼があるときだけ）。
...
また、本文の地の文（紐付け以外の参照）に `Closes #別番号` を書くと、別 Issue まで自動クローズ対象に拾われます。紐付け以外は「#NN 対応」のようにキーワード無しで書きます。
